...
test('DEBL-5 | Try to place an order without products in the cart',
    { tag: ['@DEBL-5', '@smoke', '@regression'] },
    async ({ homePage, productDetailsPage, cartPage }) => {
        ...
        await test.step('Go to cart and click on Place Order button', async () => {
            await productDetailsPage.header.cartBtn.click();
            await expect(cartPage.placeOrderBtn.first()).toBeVisible();
            await cartPage.placeOrderBtn.click();
        });
        ...
        await test.step('Click on Purchase button and verify that order cannot be placed without items in cart', async () => {
            await cartPage.purchaseBtn.click();
            await expect(cartPage.orderConfirmationMessage, 'Order was placed even when there were no products in the cart').toBeHidden();
        });
